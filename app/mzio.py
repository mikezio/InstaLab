...
LOGIN_PASSWORD = require("MZIO_LOGIN_PASSWORD")  # must be set in env/.env
TARGET_USERNAME = os.getenv("MZIO_TARGET_USERNAME", "mzio")
COOKIE_FILE = os.getenv("MZIO_COOKIE_FILE", "cookies_mzio.txt")
DB_PATH = os.getenv("MZIO_DB_PATH", "instaloader.db")
...
# Non-followbacks
non_followbacks = run["non_followbacks"]
print(f"\nAccounts you follow who DON'T follow you back: {len(non_followbacks)}")
if non_followbacks:
    for user in non_followbacks:
        print(f"  - {user}")
else:
